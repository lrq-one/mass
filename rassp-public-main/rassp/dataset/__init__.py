...
class DBDataset: #从数据库按 ID 懒加载分子和谱，适合大规模训练集
    def __init__(
        self,
        db_filename,
        db_id_field,
...
class ParquetDataset: #直接加载 .parquet 文件，适合小数据集或预处理后的数据
    def __init__(
        self,
        filename,
        spect_bin_config,
    ...
    def __getitem__(self, idx):
        ...
        collision_energy = row.get('collision_energy', None)
        adduct = row.get('adduct', None)
        
        # Simple adduct mapping
        adduct_map = {'[M+H]+': 0, '[M+Na]+': 1, '[M+K]+': 2, '[M-H]-': 3}
        adduct_idx = adduct_map.get(adduct, 0) if isinstance(adduct, str) else 0

        features_dict = self.featurizer(mol, collision_energy=collision_energy, adduct_type=adduct_idx)
...
class WrapperDataset: #仅处理分子（无谱），适合预训练或特征提取 纯推理用，只做特征提取，不包含真实谱标签
    """
...
def filter_db_records(dataset_config, cv_splitter): #根据分子属性（如分子量、原子数）和 CV 分割策略筛选数据库记录
    """
