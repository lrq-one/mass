...
def array_to_conf(mat):#将 3D 坐标矩阵转为 RDKit 构象对象
    """
...
def numpy(x): #统一将 PyTorch tensor 或列表转为 numpy 数组
   """
...
def move(tensor, cuda=False): #将 PyTorch Tensor 移动到 CPU 或 GPU
...
def morgan4_crc32(m): #计算 Morgan4 指纹的 CRC32 校验和，用于快速去重
   mf = Chem.rdMolDescriptors.GetHashedMorganFingerprint(m, 4)
   crc = zlib.crc32(mf.ToBinary())
   return crc
 
def get_formula(mol): #获取分子元素组成（原子序数:数量）
    """
...
def fast_multi_onehot(x, oh_offsets, out_array, accum=False): #快速结构化 one-hot 编码器
    """
...
def spect_to_bins(s, bins): #将质谱转换为直方图（归一化）
    """
...
def percentile(n): #生成 pandas agg 函数的百分位计算器
    """
...
def read_molfile(path): #读取分子文件（支持 .spect.sqlite 和 .parquet）
