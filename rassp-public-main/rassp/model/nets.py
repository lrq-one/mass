...
class SinusoidalEncoding(nn.Module):
    """
    Sinusoidal positional encoding for continuous values like Collision Energy.
    """
    def __init__(self, d_model, max_val=200):
...
class AdductEmbedding(nn.Module):
    """
    Simple embedding layer for adduct types.
    """
    def __init__(self, num_adducts, embedding_dim):
        super(AdductEmbedding, self).__init__()
        self.embedding = nn.Embedding(num_adducts, embedding_dim)

    def forward(self, x):
        # x: (batch_size,) containing adduct indices
