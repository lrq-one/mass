...
class SDPLoss(nn.Module):
    """
    Differentiable Stein Dot Product (SDP) Loss.
    Maximizes 1 - SDP(true, pred).
    """
    def __init__(self, mass_pow=3.0, intensity_pow=0.6, eps=1e-8, **kwargs):
        super(SDPLoss, self).__init__()
        self.mass_pow = mass_pow
        self.intensity_pow = intensity_pow
        self.eps = eps

    def forward(self, res, true_spect, input_mask_t=None, **kwargs):
        pred_spect = res['spect']
        BATCH_N, SPECT_N = pred_spect.shape

        # Create mass weighting vector
        mass_weights = torch.arange(SPECT_N, device=pred_spect.device).float() ** self.mass_pow
        mass_weights = mass_weights.unsqueeze(0) # (1, SPECT_N)

        # Apply intensity power and mass weighting
        # We use a small epsilon for stability with power < 1
        true_w = (true_spect + self.eps) ** self.intensity_pow * mass_weights
        pred_w = (pred_spect + self.eps) ** self.intensity_pow * mass_weights

        # Normalize across the bin dimension
        true_norm = torch.norm(true_w, p=2, dim=1, keepdim=True) + self.eps
        pred_norm = torch.norm(pred_w, p=2, dim=1, keepdim=True) + self.eps

        true_w_normed = true_w / true_norm
        pred_w_normed = pred_w / pred_norm

        # Stein Dot Product is the dot product of normalized weighted spectra
        sdp = torch.sum(true_w_normed * pred_w_normed, dim=1)
        
        # Loss is 1 - average SDP
        return 1.0 - sdp.mean()
