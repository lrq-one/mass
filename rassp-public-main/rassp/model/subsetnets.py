...
#思路：枚举分子顶点子集（通过断键生成候选碎片集）→ GNN 提取特征 → 对每个子集打分（这个子集产生该峰的概率）→ 聚合得到完整谱
...
class MSSubsetNet(SubsetsSampleWeightedFormulaGRUNewSparse):
    """
    MS-SubsetNet: Specialized for MS/MS prediction.
    Incorporates Collision Energy (CE) and Adduct embeddings.
    """
    def __init__(self, g_feat_in, spect_bin, 
                 ce_emb_dim=32, 
                 num_adducts=10, 
                 adduct_emb_dim=16,
                 **kwargs):
        
        super(MSSubsetNet, self).__init__(g_feat_in=g_feat_in, spect_bin=spect_bin, **kwargs)
        
        self.ce_encoding = SinusoidalEncoding(d_model=ce_emb_dim)
        self.adduct_embedding = AdductEmbedding(num_adducts=num_adducts, embedding_dim=adduct_emb_dim)
        
        # Re-initialize the first linear layer to accept the extra dimensions
        orig_input_dim = 3 * g_feat_in if self.concat_feats else g_feat_in
        self.f_combine_l1 = nn.Linear(orig_input_dim + ce_emb_dim + adduct_emb_dim, self.f_combine_l1.out_features)
    ...
    def forward(self, vert_feat_in, vert_mask_in,
        ...
        # Vertex features -> Subset features
        masked_vert_feat = vert_feat_in * vert_mask_in.unsqueeze(-1)
        atom_subsets_masked = atom_subsets * vert_mask_in.unsqueeze(1)
        atom_subsets_masked_float = atom_subsets_masked.float()
        sample_weighted_vert_sum = torch.einsum("ijk,ilj->ilk", masked_vert_feat.float(), atom_subsets_masked_float)
        ...
        possible_formulae_flat = possible_formulae.reshape(-1, possible_formulae.shape[-1])
        possible_formulae_flat_oh = self.formula_to_oh(possible_formulae_flat)
        possible_formulae = possible_formulae_flat_oh.reshape(possible_formulae.shape[0], possible_formulae.shape[1], -1)

        if self.possible_formulae_norm:
            possible_formulae = self.pf_norm(possible_formulae.reshape(-1, possible_formulae.shape[-1])).reshape(possible_formulae.shape)

        eps = 1e-4
        subset_size = atom_subsets_masked_float.sum(dim=2) + eps
        subset_normed = self.subset_weighted_norm(sample_weighted_vert_sum / subset_size.unsqueeze(-1))
        ...
        # MS/MS Injection: CE and Adduct
        if collision_energy is not None:
            ce_feats = self.ce_encoding(collision_energy) # (BATCH_N, ce_emb_dim)
            x = torch.cat([x, ce_feats.unsqueeze(1).repeat(1, SUBSET_SAMPLES_N, 1)], dim=-1)
        
        if adduct_type is not None:
            adduct_feats = self.adduct_embedding(adduct_type) # (BATCH_N, adduct_emb_dim)
            x = torch.cat([x, adduct_feats.unsqueeze(1).repeat(1, SUBSET_SAMPLES_N, 1)], dim=-1)
        ...
        # Global Softmax (Competition)
        if self.mask_zero_subsets:
            nonzero_atom_subsets = (atom_subsets.sum(dim=2) != 0).float()
            formulae_scores = formulae_scores + (-1000 * (1 - nonzero_atom_subsets))
        
        formulae_probs = torch.softmax(formulae_scores, dim=-1)

        # Aggregate to Spectrum
        with torch.cuda.amp.autocast(enabled=False):
            out_y = []
            for batch_i in range(BATCH_N):
                sparse_mat_matrix = peak_indices_intensities_to_sparse_matrix(
                    kwargs['atom_subsets_peaks_mass_idx'][batch_i],
                    kwargs['atom_subsets_peaks_intensity'][batch_i], 
                    self.spect_bin.get_num_bins()
                )
                y = sparse_mat_matrix @ formulae_probs[batch_i].float()
                out_y.append(y)
            spect_out = torch.stack(out_y)
        ...
        return {'spect': spect_out, 'subset_probs': formulae_probs}
