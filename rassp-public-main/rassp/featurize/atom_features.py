...
[in contrast to whole-molecule featurizations]
 逐原子特征提取
 核心函数 feat_tensor_atom()：将一个 RDKit 分子对象转化为形状 (原子数 × 特征维度) 的张量，特征包括：
 特征	说明
feat_atomicno	原子序数（整数）
feat_atomicno_onehot	对 [H,C,N,O,F,...] 做 One-Hot
feat_valence	总化合价
hybridization	杂化类型（sp, sp2, sp3...）的 One-Hot
aromatic	是否芳香性
formal_charge	正式电荷（-1/0/+1）的 One-Hot
r_covalent / r_vanderwals	共价半径 / 范德华半径
rings	是否在各种大小的环中
total_h_oh	连接的氢数的 One-Hot
"""
