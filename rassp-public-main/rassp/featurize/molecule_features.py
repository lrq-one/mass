...
分子级别特征（邻接矩阵等）
函数	作用
mol_to_nums_adj()	分子转对称邻接矩阵（元素为键级）
feat_mol_adj()	计算邻接矩阵（可按键级分通道、加归一化、加单位矩阵）
feat_tensor_mol()	计算分子内原子对级别特征（距离、二面角、库仑矩阵等）
get_single_bond_fragment_masses()	计算切断每条键后两个碎片的质量（N×N×2 矩阵）
PossibleFormulaFeaturizer	关键类：枚举分子所有可能的碎片化学式，并计算每种式子对应的精确质量峰，输出编码用于 FormulaNet 模型
"""
